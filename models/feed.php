...
/*=========================================================================
  ...
  Module:    $Id: feed.php 3433 2014-01-23 11:09:02Z jjomier $
  Language:  PHP
  Date:      $Date: 2014-01-23 06:09:02 -0500 (Thu, 23 Jan 2014) $
  Version:   $Revision: 3433 $
...
class Feed
{
  ...
  function InsertTest($projectid,$buildid)
    ...
    while($query_array = pdo_fetch_array($query))
      ...
      if($query_array['type'] == 1) // failed
        {
        $type = 'failed';
        $feedtype = Feed::TypeTestFailing;
        }
      else if($query_array['type'] == 2) // pass
        {
        $type = 'passing';
      ...
      if(($query_array['type']==0 || $query_array['type']==1)
        ...
        if($query_array['difference_positive']>1) {$description .= 's';}
        $description .= ' '.$type;
      ...
      if($query_array['type'] == 1 && $query_array['difference_negative']>0)
        ...
        if($query_array['difference_negative']>1) {$description .= 's';}
  ...
  function InsertBuild($projectid,$buildid)
    ...
    while($query_array = pdo_fetch_array($query))
      ...
      else if($query_array['difference_negative']>0)
        {
  ...
  function InsertUpdate($projectid,$buildid)
    {
    ...
    if($nfiles == 0) // no updates we return;
      {
      return;
      }

    $description = $nfiles.' file';
    if($nfiles > 1) {$description .= 's';}
  ...
  function GetFeed($projectid,$limit)
    {
    if(!$projectid)
      {

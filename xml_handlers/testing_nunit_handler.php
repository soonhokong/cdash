...
/*=========================================================================
  ...
  Module:    $Id: testing_nunit_handler.php 3287 2012-11-28 13:37:29Z jjomier $
  Language:  PHP
  Date:      $Date: 2012-11-28 08:37:29 -0500 (Wed, 28 Nov 2012) $
...
class TestingNUnitHandler extends AbstractHandler
{
  ...
  public function startElement($parser, $name, $attributes)
    ...
    if($name=='SITE')
    ...
    else if($name == "TEST-CASE" && count($attributes) > 0)
      {
      $this->Test = new Test();
      ...
      else if($attributes['RESULT'] == "Failure")
        {
        $teststatus = "failed";
        }
    ...
    else if($name == "TEST-RESULTS")
      {
      $this->StartTimeStamp = strtotime($attributes['DATE'].' '.$attributes['TIME']);
      $this->EndTimeStamp = $this->StartTimeStamp;
      }
    else if($this->HasSiteTag == false && $name == "ENVIRONMENT")
      {
    ...
    else if($this->BuildAdded == false && $name == "TEST-SUITE")
      {
  ...
  public function endElement($parser, $name)
    ...
    else if($name == "SITE" || ($this->HasSiteTag == false && $name == "TEST-RESULTS"))
      {

...
/*=========================================================================
  ...
  Module:    $Id: index.php 3222 2012-02-21 12:26:55Z jjomier $
  Language:  PHP
  Date:      $Date: 2012-02-21 13:26:55 +0100 (mar., 21 févr. 2012) $
  Version:   $Revision: 3222 $
...
function generate_index_table()
{
  ...
  if(!pdo_query("SELECT updateid FROM build2update LIMIT 1") )
    {
...
function generate_main_dashboard_XML($projectid,$date)
{
  ...
  foreach($build_rows as $build_array)
    ...
    if(!empty($labels_array) || !empty($build_array['updatestarttime']) ||
       $build_array['countbuilderrors']>=0 || $build_array['hasconfigurestatus'] != 0 ||
       $build_array['hastest'] != 0) 
      {
      $xml .= "<build>";
